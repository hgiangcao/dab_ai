...
def worker_execute_episode_chunk(worker_args):
    ...
    opponent_cache = _WORKER_SHARED_STATE.setdefault("opponent_cache", {})

    def get_opponent(opp_type, opp_path):
        import os
        sig = None
        if opp_path and os.path.exists(opp_path):
            sig = (os.path.getmtime(opp_path), os.path.getsize(opp_path))
            
        key = (opp_type, opp_path, sig)
        if key in opponent_cache:
            return opponent_cache[key]
            
        # Clear old cached models for this opp_type to save memory
        for k in list(opponent_cache.keys()):
            if k[0] == opp_type:
                del opponent_cache[k]

        if opp_type in ["self", "past"]:
    ...
    def run_episode(opp_type, opp_path, prefilled_moves=None):
        ...
        while game.is_running():
            ...
            if move_number < 4:
                ...
                if is_latest_turn or opp_type in ["self", "best"]:
                    pi = np.zeros(game.N_LINES, dtype=np.float32)
                    for m in valid_moves:
                        pi[m] = 1.0 / len(valid_moves)
                        
                    canonical_lines = game.get_canonical_lines()
                    canonical_boxes = game.get_canonical_boxes()
                    agent_boxes     = int(np.sum(game.b == game.current_player))
                    opponent_boxes  = int(np.sum(game.b == -game.current_player))
                ...
                if is_latest_turn or agent_opp is None:
                ...
                if is_latest_turn or opp_type in ["self", "best"]:
                    ...
                    # Snapshot cumulative box score (from canonical perspective: +1 = our box)
                    # current_player is the agent whose turn it is.
                    agent_boxes     = int(np.sum(game.b == game.current_player))
                    opponent_boxes  = int(np.sum(game.b == -game.current_player))
                    train_examples.append([
                        canonical_lines,
                        canonical_boxes,
                        game.current_player,   # used for terminal value sign flip
                        pi,
                        agent_boxes,           # cumulative boxes for current player
                        opponent_boxes         # cumulative boxes for opponent
                    ])
